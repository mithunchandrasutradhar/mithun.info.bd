...
if ($mode == 'edit' || $mode == 'delete') {
  ...
  $item = mysqli_query($con, "SELECT * FROM `training` WHRE WHERE id = $id");
  if (mysqli_num_rows($item) < 1) {
    die("Empty Training Information");
  }
...
if ($mode == 'add') { ?>
  ...
  <form action="./submit/training.php?action=add" method="POST" autocomplete="off">
...
?>
  ...
  <form action="./submit/training.php?action=edit" method="POST" autocomplete="off">
...
?>
  ...
  <form action="./submit/training.php?action=delete" method="POST" autocomplete="off">
    <input type="hidden" name="id" value="<?= $id ?>">
    <div class="modal-body">
      <label><strong>Are you sure to delete this Training!</strong></label>
