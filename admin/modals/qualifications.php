...
if ($mode == 'edit' || $mode == 'delete') {
  ...
  $item = mysqli_query($con, "SELECT * FROM `qualifications` WHRE WHERE id = $id");
  if (mysqli_num_rows($item) < 1) {
    die("Empty Qualification Information");
  }
...
<form action="./submit/qualifications.php?action=add" method="POST" autocomplete="off">
...
<form action="./submit/qualifications.php?action=edit" method="POST" autocomplete="off">
...
<form action="./submit/qualifications.php?action=delete" method="POST" autocomplete="off">
  <input type="hidden" name="id" value="<?= $id ?>">
  <div class="modal-body">
    <label><strong>Are you sure to delete this Qualification!</strong></label>
